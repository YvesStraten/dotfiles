...
(defun ys/new-persp-switch (dir)
  (persp-new (ys/get-dir-name dir))
  (persp-switch (ys/get-dir-name dir))
  (cd dir)
  (project-find-file)
  )
...
(use-package dashboard
  ...
  (setq dashboard-heading-icons '((recents   . "history")
                                  ...
                                  (projects  . "rocket")
                                  (registers . "database")))
  (setq dashboard-projects-switch-function 'ys/new-persp-switch
        dashboard-items '(
                          (recents . 5)
                          (projects . 10)
                          )
...
(defun ys/get-dir-name (path)
  (file-name-nondirectory (directory-file-name (file-name-directory path)))
  )

(defun ys/project-new-persp ()
  (interactive)
  (let ((selected (project-prompt-project-dir)))
    (ys/new-persp-switch selected)
    )
  )
...
(defun ys/project-find-or-switch ()
  ...
  (if (project-current)
      (project-find-file)
    (call-interactively 'ys/project-new-persp)))

(defun ys/project-rg ()
  (interactive)
  (counsel-rg nil nil nil "What to search for: ")
  )

(use-package project
  :ensure nil
  :config
  (setq project-prompter
        #'ivy-read)
  :general
  (ys/leader-keys
    "pc" '(project-compile :wk "Compile project")
    "ff" '(ys/project-find-or-switch :wk "Find file")
    "pp" '(ys/project-new-persp :wk "Switch project")
...
(use-package yasnippet
  ...
  :defer 1
  :config
  (setq yas-snippet-dirs (cons
                          (concat (file-name-directory user-init-file) "snippets")
                          yas-snippet-dirs))
...
(use-package emacs
  ...
  (+macos--open-with reveal-project-in-finder "Finder" (or (project-root) default-directory))
